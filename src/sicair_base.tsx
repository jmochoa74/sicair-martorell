...
// ── Demo calidad efluente ──────────────────────────────────────────
function generarDemoCalidad() {
  const now = new Date();
  return Array.from({length:20}, (_,i) => {
    const dt = new Date(now.getTime() - (20-i)*2*86400000);
    const nh4 = +(Math.max(0.5, 4 + (Math.random()-0.5)*6)).toFixed(2);
    const no3 = +(Math.max(2, 18 + (Math.random()-0.5)*20)).toFixed(2);
    return { id: dt.getTime(), fecha: dt.toISOString().slice(0,10), nh4, no3, obs: "" };
  });
}
      ...
      {vistadiag==="manual"&&(
        <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:10,padding:"20px 24px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
            <div style={{fontSize:14,fontWeight:700}}>📋 Historial de Incidencias</div>
            {incidencias.length>0&&<button onClick={()=>exportJSON(incidencias,`incidencias_sicair_${new Date().toISOString().slice(0,10)}.json`)} style={{background:C.blue,color:"#fff",border:"none",borderRadius:7,padding:"5px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📥 Exportar JSON</button>}
          </div>
...
function OnboardingPanel({reactores,pred,historico,deriva,onSkip,handleCSV,handleXLSX,handleJSON,parseJSON,setHistorico,setDeriva,nombres,addingR,setAddingR,newRN,setNewRN,incidencias,setIncidencias,calidad,setCalidad}) {
  const pasos = [
  ...
  return (
    <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:12,padding:"24px 28px",marginBottom:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:10}}>
        ...
        <DropZone onFile={raw=>{const j=parseJSON(raw);if(Array.isArray(j)){setIncidencias(j);lsSet("incidencias_sicair",j);}}} accept=".json" color={C.muted} done={incidencias.length>0} label="incidencias_sicair.json" sublabel={incidencias.length>0?`${incidencias.length} incidencias`:"Opcional · sincroniza desde Drive"}/>
        <DropZone onFile={raw=>{const j=parseJSON(raw);if(Array.isArray(j)){setCalidad(j);lsSet("calidad_efluente",j);}}} accept=".json" color={C.blue} done={calidad.length>0} label="calidad_efluente.json" sublabel={calidad.length>0?`${calidad.length} registros`:"Opcional · NH₄ y NO₃ histórico"}/>
      </div>
...
const NH4_LIMITES = [{max:5,label:"Excelente",color:"#4a9c3f"},{max:10,label:"Correcto",color:"#6dba5f"},{max:20,label:"Vigilancia",color:"#e65100"},{max:Infinity,label:"Alerta",color:"#c62828"}];
const NO3_LIMITES = [{max:10,label:"Excelente",color:"#4a9c3f"},{max:25,label:"Correcto",color:"#6dba5f"},{max:50,label:"Vigilancia",color:"#e65100"},{max:Infinity,label:"Alerta",color:"#c62828"}];
function colorParam(val, limites) { return (limites.find(l=>val<=l.max)||limites.at(-1)).color; }
function labelParam(val, limites) { return (limites.find(l=>val<=l.max)||limites.at(-1)).label; }

function CalidadEfluentePanel({registros, setRegistros}) {
  ...
  const guardar = () => {
    const n4=parseFloat(nh4.replace(",",".")), n3=parseFloat(no3.replace(",","."));
    if (isNaN(n4)&&isNaN(n3)) return;
    const nuevo = {id:Date.now(), fecha, nh4:isNaN(n4)?null:+n4.toFixed(2), no3:isNaN(n3)?null:+n3.toFixed(2), obs:obs.trim()};
    const nuevos = [nuevo, ...registros].sort((a,b)=>b.fecha.localeCompare(a.fecha));
    setRegistros(nuevos);
    lsSet("calidad_efluente", nuevos);
    setNh4(""); setNo3(""); setObs("");
  };
  const eliminar = id => {
  ...
  const exportar = () => exportJSON(registros, `calidad_efluente_${HOY}.json`);

  const ultimos = registros.slice(0,1)[0];
  const nh4Col = ultimos?.nh4!=null ? colorParam(ultimos.nh4, NH4_LIMITES) : C.muted;
  const no3Col = ultimos?.no3!=null ? colorParam(ultimos.no3, NO3_LIMITES) : C.muted;

  // Datos para gráfico — ordenados cronológicamente
  const grafData = [...registros].sort((a,b)=>a.fecha.localeCompare(b.fecha)).slice(-30).map(r=>({
  ...
  return (
    <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:10,padding:"20px 24px",marginBottom:14}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:14,fontWeight:700}}>🧪 Calidad Efluente</div>
          <div style={{fontSize:11,color:C.muted,marginTop:2}}>NH₄ y NO₃ en salida · cada 2 días</div>
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={()=>setVista(v=>v==="grafico"?"tabla":"grafico")} style={{background:"#fff",color:C.muted,border:`1px solid ${C.border}`,borderRadius:7,padding:"5px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
            {vista==="grafico"?"📋 Ver tabla":"📈 Ver gráfico"}
          </button>
          {registros.length>0&&<button onClick={exportar} style={{background:C.blue,color:"#fff",border:"none",borderRadius:7,padding:"5px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📥 Exportar JSON</button>}
        </div>
      </div>
      ...
      {ultimos && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
          <div style={{background:"#fff",border:`2px solid ${nh4Col}44`,borderRadius:10,padding:"14px 18px",borderTop:`3px solid ${nh4Col}`}}>
            <div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>NH₄ efluente · {ultimos.fecha}</div>
            <div style={{fontSize:28,fontWeight:900,color:nh4Col,fontFamily:"monospace"}}>{ultimos.nh4!=null?`${ultimos.nh4} mg/L`:"—"}</div>
            <div style={{fontSize:11,fontWeight:700,color:nh4Col,marginTop:4}}>{ultimos.nh4!=null?labelParam(ultimos.nh4,NH4_LIMITES):"Sin dato"}</div>
            <div style={{fontSize:10,color:C.muted,marginTop:2}}>Límite vertido: ≤10 mg/L</div>
          </div>
          <div style={{background:"#fff",border:`2px solid ${no3Col}44`,borderRadius:10,padding:"14px 18px",borderTop:`3px solid ${no3Col}`}}>
            <div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>NO₃ efluente · {ultimos.fecha}</div>
            <div style={{fontSize:28,fontWeight:900,color:no3Col,fontFamily:"monospace"}}>{ultimos.no3!=null?`${ultimos.no3} mg/L`:"—"}</div>
            <div style={{fontSize:11,fontWeight:700,color:no3Col,marginTop:4}}>{ultimos.no3!=null?labelParam(ultimos.no3,NO3_LIMITES):"Sin dato"}</div>
            <div style={{fontSize:10,color:C.muted,marginTop:2}}>Referencia: ≤50 mg/L</div>
          </div>
        </div>
      )}
      ...
      {/* Formulario */}
      <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:10,padding:"16px",marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:12}}>➕ Nuevo registro</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
          ...
          <div>
            <div style={{fontSize:11,color:C.muted,marginBottom:4}}>NH₄ efluente (mg/L)</div>
            <input type="number" step="0.01" min="0" placeholder="ej. 3.2" value={nh4} onChange={e=>setNh4(e.target.value)}
              style={{width:"100%",border:`1px solid ${nh4&&!isNaN(parseFloat(nh4))?colorParam(parseFloat(nh4),NH4_LIMITES):C.border}`,borderRadius:6,padding:"6px 10px",fontSize:13,fontWeight:700,boxSizing:"border-box"}}/>
          </div>
          <div>
            <div style={{fontSize:11,color:C.muted,marginBottom:4}}>NO₃ efluente (mg/L)</div>
            <input type="number" step="0.1" min="0" placeholder="ej. 18.5" value={no3} onChange={e=>setNo3(e.target.value)}
              style={{width:"100%",border:`1px solid ${no3&&!isNaN(parseFloat(no3))?colorParam(parseFloat(no3),NO3_LIMITES):C.border}`,borderRadius:6,padding:"6px 10px",fontSize:13,fontWeight:700,boxSizing:"border-box"}}/>
          </div>
        </div>
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:C.muted,marginBottom:4}}>Observaciones (opcional)</div>
          <input type="text" placeholder="p.ej. muestra tras lluvia intensa" value={obs} onChange={e=>setObs(e.target.value)}
            style={{width:"100%",border:`1px solid ${C.border}`,borderRadius:6,padding:"6px 10px",fontSize:13,boxSizing:"border-box"}}/>
        </div>
        <button onClick={guardar} disabled={!nh4&&!no3} style={{background:(nh4||no3)?C.green:"#ccc",color:"#fff",border:"none",borderRadius:8,padding:"8px 20px",fontSize:13,fontWeight:700,cursor:(nh4||no3)?"pointer":"default"}}>
          ✅ Guardar
        </button>
      </div>
      ...
      <div style={{display:"flex",gap:12,marginBottom:14,flexWrap:"wrap"}}>
        <div style={{fontSize:11,color:C.muted,fontWeight:700}}>NH₄:</div>
        {NH4_LIMITES.map((l,i) => <span key={i} style={{fontSize:10,color:l.color,fontWeight:700}}>≤{l.max===Infinity?"∞":l.max} {l.label}</span>)}
        <div style={{width:1,background:C.border,margin:"0 4px"}}/>
        <div style={{fontSize:11,color:C.muted,fontWeight:700}}>NO₃:</div>
        {NO3_LIMITES.map((l,i) => <span key={i} style={{fontSize:10,color:l.color,fontWeight:700}}>≤{l.max===Infinity?"∞":l.max} {l.label}</span>)}
      </div>

      {registros.length===0 && (
        <div style={{textAlign:"center",padding:"24px 0",color:C.muted,fontSize:13}}>Sin registros todavía. Introduce el primer dato de calidad.</div>
      )}

      {/* Gráfico */}
      {vista==="grafico" && grafData.length>1 && (
        <div>
          <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:8}}>Últimos {grafData.length} registros</div>
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={grafData} margin={{top:4,right:8,bottom:4,left:0}}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.gridLine}/>
              <XAxis dataKey="label" tick={{fontSize:9,fill:C.muted}} interval={Math.floor(grafData.length/6)} angle={-20} textAnchor="end" height={36}/>
              <YAxis yAxisId="nh4" domain={[0,"auto"]} tick={{fontSize:9,fill:C.muted}} label={{value:"NH₄",angle:-90,position:"insideLeft",fontSize:9,fill:C.muted}}/>
              <YAxis yAxisId="no3" orientation="right" domain={[0,"auto"]} tick={{fontSize:9,fill:C.muted}} label={{value:"NO₃",angle:90,position:"insideRight",fontSize:9,fill:C.muted}}/>
              <Tooltip content={<CT/>}/><Legend wrapperStyle={{fontSize:11}}/>
              <Line yAxisId="nh4" dataKey="nh4" stroke={C.green} strokeWidth={2} dot={{r:4,fill:C.green}} name="NH₄ (mg/L)" connectNulls/>
              <Line yAxisId="no3" dataKey="no3" stroke={C.blue} strokeWidth={2} dot={{r:4,fill:C.blue}} name="NO₃ (mg/L)" connectNulls strokeDasharray="5 3"/>
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {/* Tabla */}
      {vista==="tabla" && registros.length>0 && (
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead>
              <tr style={{borderBottom:`2px solid ${C.border}`}}>
                {["Fecha","NH₄ (mg/L)","Estado NH₄","NO₃ (mg/L)","Estado NO₃","Obs.",""].map((h,i)=>(
            ...
            <tbody>
              {registros.map(r=>(
                <tr key={r.id} style={{borderBottom:`1px solid ${C.gridLine}`}}>
                  ...
                  <td style={{padding:"8px 10px",fontWeight:800,fontFamily:"monospace",color:r.nh4!=null?colorParam(r.nh4,NH4_LIMITES):C.muted}}>{r.nh4??"—"}</td>
                  <td style={{padding:"8px 10px"}}>{r.nh4!=null&&<Badge color={colorParam(r.nh4,NH4_LIMITES)}>{labelParam(r.nh4,NH4_LIMITES)}</Badge>}</td>
                  <td style={{padding:"8px 10px",fontWeight:800,fontFamily:"monospace",color:r.no3!=null?colorParam(r.no3,NO3_LIMITES):C.muted}}>{r.no3??"—"}</td>
                  <td style={{padding:"8px 10px"}}>{r.no3!=null&&<Badge color={colorParam(r.no3,NO3_LIMITES)}>{labelParam(r.no3,NO3_LIMITES)}</Badge>}</td>
                  <td style={{padding:"8px 10px",color:C.muted,fontSize:11}}>{r.obs||"—"}</td>
                  <td style={{padding:"8px 10px"}}><button onClick={()=>eliminar(r.id)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:14}}>✕</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function App() {
  ...
  const [incidencias, setIncidencias] = useState(() => lsGet("incidencias_sicair", []));
  const [calidad,     setCalidad]     = useState(() => lsGet("calidad_efluente", []));
  ...
  useEffect(() => {
    ...
    setCalidad(generarDemoCalidad());
    setDemoMode(true); setShowOnboard(false); setTab("pred");
  }, []);

  const salirDemo = () => { setReactores({}); setPred(null); setHistorico(null); setDeriva(null); setCalidad(lsGet("calidad_efluente",[])); setDemoMode(false); setShowOnboard(true); setTab("pred"); };
  ...
  const tabs = [
    ...
    {id:"calidad",     label:"🧪 Calidad efluente",disabled:false, badge: calidad.length>0&&calidad[0]?.nh4!=null&&calidad[0].nh4>10?1:0, badgeColor:C.red},
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{padding:"20px 24px",maxWidth:1300,margin:"0 auto"}}>
        ...
        {tab==="incidencias" &&<IncidenciasPanel incidencias={incidencias} setIncidencias={v=>{setIncidencias(v);lsSet("incidencias_sicair",v);}} diagHistorico={diagHistorico} setDiagHistorico={setDiagHistorico} parseJSON={parseJSON}/>}
        {tab==="calidad"     &&<CalidadEfluentePanel registros={calidad} setRegistros={setCalidad}/>}
